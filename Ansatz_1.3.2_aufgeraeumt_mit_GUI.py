...
log_file = os.path.join(os.path.dirname(sys.executable), 'logfile.txt')

def log_message(message):
    with open(log_file, 'a') as f:
        f.write(message + '\n')

# Überprüfen, ob das Script als EXE ausgeführt wird und _MEIPASS gesetzt ist
if hasattr(sys, '_MEIPASS'):
    log_message(f"sys._MEIPASS Pfad: {sys._MEIPASS}")  # Schreibe sys._MEIPASS Pfad in die Log-Datei
    model_path = os.path.join(sys._MEIPASS, 'spheroid_segmentation_unet_trained3.1.h5')
    log_message(f"Laden des Modells aus: {model_path}")
else:
    model_path = 'C:/Users/chris/Documents/Master/Sphaeroidauswertung/Modelle/spheroid_segmentation_unet_trained3.1.h5'
    log_message(f"Laden des Modells aus: {model_path}")
